...
class SYSTEM():
    def __init__(self):
        ...
        self.button_auto = Button(self.root, text="AUTO(STEP)", width=10, fg="grey20", highlightbackground=BACKGROUND,
                                  command=self.auto)
        self.button_auto.place(x=machine_x, y=machine_y + 25 + (machine_y - user_y) - 50)

        self.button_auto2 = Button(self.root, text="AUTO(FULL)", width=10, fg="grey20", highlightbackground=BACKGROUND,
                                   command=self.fullauto)
    ...
    def set_new_board(self):
        ...
        print("len(available_whole_lines) : ", len(available_whole_lines))
        if (len(available_whole_lines) % 2 == 0):
            print("후공 유리")
        else:
            print("선공 유리")
